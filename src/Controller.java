...
public class Controller {
	public static void main(String[] args) throws Exception {
		 ...
		 * "2550", "3054" }; String RegimentIds[][]={
		 * {"2216","3956","7017","7673","17553","18363"}, {"12942"}, {"7465"},
		 * {"11121","15720"}, {"5623","6362","7300","14664","15158","17163"},
		 * {"18418"}, {"13219"}, {"14963","17182"} }; List<String> memberIds =
		 * new ArrayList<String>(); String baseUrl =
		 * "http://www.erepublik.com/en/main/group-list/members/"; for(int i=0;
		 * i<MUIds.length; i++){ for(int j=0; j<RegimentIds[i].length; j++){
		 * System.out.println("\n"+MUIds[i]+" "+RegimentIds[i][j]+"\n");
		 * driver.get(baseUrl+MUIds[i]+"/"+RegimentIds[i][j]);
		 * Thread.sleep(5000); List<WebElement> members =
		 * driver.findElements(By.
		 * xpath("//table[@id='member_listing']//tr[@class='members ']"));
		 * System.out.println("No. of ppl in regiment : "+members.size());
		 * for(int k=0; k<members.size(); k++) {
		 * System.out.println(members.get(k
		 * ).getAttribute("id").toString().substring(7));
		 * memberIds.add(members.get
		 * (k).getAttribute("id").toString().substring(7)); }
		 * 
		 * } } File file = new File("Erep India users.txt"); if(!file.exists())
		 * file.createNewFile(); FileWriter fw = new
		 * FileWriter(file.getAbsoluteFile()); BufferedWriter bw = new
		 * BufferedWriter(fw); for(int i=0; i<memberIds.size();i++)
